import { LocalVoiceMap, TelegramVoiceMap, VoiceMessage } from "./types";
...
export class VoicesLoader {
  private readonly localVoicesFilepath = path.resolve(
    "./assets/local_voice_map.json"
  );
  ...
  async load(): Promise<TelegramVoiceMap> {
    const localVoicesFile = fsPromises.readFile(this.localVoicesFilepath, {
      encoding: "utf-8",
    });
  ...
  private async uploadNewVoices(
    files: LocalVoiceMap,
    voiceMap: TelegramVoiceMap,
    newVoiceMap: TelegramVoiceMap
  ) {
